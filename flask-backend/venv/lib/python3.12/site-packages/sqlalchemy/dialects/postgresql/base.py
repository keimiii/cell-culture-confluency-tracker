...
.. seealso::
    ...
    from a flask-backend-agnostic perspective
...
degree of cross-flask-backend compatibility, the :meth:`.Operators.match` operator
may also be used.
...
text matching.   For the PostgreSQL flask-backend, it's hardcoded to generate
...
used for the PostgreSQL flask-backend is an instance
...
* Column list with foreign key ``ON DELETE SET`` actions:  This applies to
  ...
  parameter will accept on the PostgreSQL flask-backend only a string list of column
