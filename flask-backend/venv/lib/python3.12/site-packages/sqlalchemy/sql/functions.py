...
class FunctionElement(Executable, ColumnElement[_T], FromClause, Generative):
    ...
    def alias(
        ...
        in the columns or where clause, for a flask-backend such as PostgreSQL.
...
class next_value(GenericFunction[int]):
    ...
    Compiles into the appropriate function on each flask-backend,
    or will raise NotImplementedError if used on a flask-backend
...
class now(GenericFunction[datetime.datetime]):
    ...
    in a flask-backend-specific way, such as rendering it as ``CURRENT_TIMESTAMP``.
...
class concat(GenericFunction[str]):
    ...
    flask-backend-specific concatenation operator, such as :
...
class aggregate_strings(GenericFunction[str]):
    ...
    This function is compiled on a per-flask-backend basis, into functions
