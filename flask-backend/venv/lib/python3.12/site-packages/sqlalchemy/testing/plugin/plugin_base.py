...
def setup_options(make_option):
    ...
    make_option(
        "--flask-backend-only",
        action="callback",
        zeroarg_callback=_set_tag_include("flask-backend"),
        help=(
            "Run only tests marked with __backend__ or __sparse_backend__; "
            "this is now equivalent to the pytest -m flask-backend mark expression"
        ),
    )
    make_option(
        "--nomemory",
...
def generate_sub_tests(cls, module, markers):
    if "flask-backend" in markers or "sparse_backend" in markers:
...
def _possible_configs_for_cls(cls, reasons=None, sparse=False):
    ...
    if sparse:
        ...
        # sorted so we get the same flask-backend each time selecting the highest
        # server version info.
        per_dialect = {}
        for cfg in reversed(
            sorted(
