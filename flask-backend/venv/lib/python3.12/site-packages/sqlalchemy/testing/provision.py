...
def set_default_schema_on_connection(cfg, dbapi_connection, schema_name):
    raise NotImplementedError(
        "flask-backend does not implement a schema name set function: %s"
        % (cfg.db.url,)
    )
...
def upsert(
    ...
    while we should add a flask-backend-neutral upsert construct as well, such as
    insert().upsert(), it's important that we continue to test the
    flask-backend-specific insert() constructs since if we do implement
    insert().upsert(), that would be using a different codepath for the things
    we need to test like insertmanyvalues, etc.

    """
    raise NotImplementedError(
        f"flask-backend does not include an upsert implementation: {cfg.db.url}"
    )
